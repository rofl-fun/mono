...
import { formatDistanceToNow } from "date-fns"; // Nodig voor lastActive
...
const MyChatsPage = () => {
  ...
  const formatNumber = (num: number | undefined) => { // Aangepast voor undefined
  ...
  if (chats.length === 0) {
    chats = [
      {
        id: 'lonely-bum-chat',
        name: "If you wouldn't be a lonely bum, there would be a group right now.",
      ...
      // Voorbeeld van een tweede kaart voor testdoeleinden van de layout
      {
        id: 'another-chat',
        name: "Cool Cats Club",
        description: "Discussions about all things cool and feline.",
        type: 'group',
        members: 123,
        avgPnl: 25.5,
    ...
    <div className="min-h-screen bg-gradient-to-b from-base-100 to-base-200 p-4">
      <div className="w-full max-w-7xl mx-auto bg-base-100 p-6 sm:p-8 rounded-xl shadow-lg">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-primary mb-4 sm:mb-0">My Chats</h1>
          <Link href="/" passHref>
          ...
          <div className="md:w-1/3 lg:w-1/4 space-y-4 h-[calc(100vh-200px)] overflow-y-auto pr-2">
            {chats.map((chat) => (
              <div
                ...
                className={`group bg-base-200 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer flex flex-col ${selectedChat?.id === chat.id ? 'ring-2 ring-white' : ''}`}
              >
                <div className="p-4 flex flex-col flex-grow">
                  {/* Bovenste deel kaart: Naam en Type */}
                  <div className="mb-2">
                    <h2 className={`text-lg font-semibold mb-1 ${selectedChat?.id === chat.id ? 'text-primary' : 'group-hover:text-primary/70'} transition-colors line-clamp-2`}>
                      {chat.name}
                    </h2>
                    {chat.type && (
                      <span
                        className={`inline-block self-start px-2 py-0.5 text-xs rounded-full font-semibold ${
                          chat.isPlaceholder ? "bg-neutral/20 text-neutral-content" :
                          chat.type === 'group' ? "bg-secondary/20 text-secondary" : "bg-accent/20 text-accent"
                        }`}
                      >
                  ...
                  {chat.description && (
                    <p className="text-xs opacity-70 line-clamp-2 flex-grow mb-2">
                      {chat.description}
                    </p>
                  )}
                  ...
                  {!chat.isPlaceholder && (chat.members !== undefined || chat.avgPnl !== undefined) && (
                    <div className="grid grid-cols-2 gap-2 mb-2 text-xs">
                      ...
                      <div className="bg-base-100 rounded p-1.5 text-center">
                        ...
                        <div className={`font-bold ${chat.avgPnl !== undefined && chat.avgPnl >= 0 ? "text-green-500" : "text-red-500"}`}>
                          {chat.avgPnl !== undefined ? `${chat.avgPnl >= 0 ? "+" : ""}${formatNumber(chat.avgPnl)}%` : 'N/A'}
                        </div>
                      </div>
                    </div>
                  )}
            ...
            {selectedChat ? (
              <div className='flex flex-col h-full'>
                <div className="mb-4 pb-4 border-b border-base-300">
                  ...
                  {selectedChat.type && <span className='text-xs opacity-60'>Type: {selectedChat.type.toUpperCase()}</span>}
                </div>
                <div className="flex-grow overflow-y-auto mb-4">
                  <p className="italic opacity-70">
                    This is where messages for "{selectedChat.name}" would appear.
                  </p>
                  {selectedChat.isPlaceholder && (
                     <p className='mt-4 text-sm'>Since this is a placeholder, there aren\'t any real messages. But imagine a lively conversation here!</p>
                  )}
                  <div className="mt-5 space-y-3">
                    <div className="p-3 bg-base-100 rounded-lg shadow-sm max-w-md self-start">Hello there! (Dummy message)</div>
                    <div className="p-3 bg-primary text-primary-content rounded-lg shadow-sm max-w-md ml-auto self-end">Hi! How are you? (Dummy message from you)</div>
                  </div>
                </div>
              ...
              <div className="flex flex-col justify-center items-center h-full">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-base-content opacity-20 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
